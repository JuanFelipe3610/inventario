import {getCookie, setCookie, checkCookie} from '/resources/js/modules/cookies.js';
...
export function ajax(URL, DATA, FN) {
    $.ajax({
        type: "POST",
        url: URL,
        async: false,
...
$(document).ready(function() {
    ...
    function createWindow(){
        ...
        let selected = li[li.length - 1]
        let id = parseInt($(selected).attr('data-id')) + 1
        let winButton = '<li class="active" data-id="'+id+'">Nueva ventana <i class="icon_close"></i></li>'
        let windows = '<div class="content active" data-id="'+id+'">Nueva ventana '+id+'</div>'
    ...
    function changeWindow(element){
        let content = $('.main-content')
        let child = content.children('div')
        let ul = $('.window')
        let li = ul.children('li')
